...
parser.add_argument('--seed',       type=int,   default=1106,             help='Random seed')
parser.add_argument('--arch',       type=str,   default='vaeIdsiaStn',  help='network type: vaeIdsia, vaeIdsiaStn')
parser.add_argument('--dataset',    type=str,   default='belga2toplogo', help='dataset to use [gtsrb, belga2flickr, belga2toplogo]') # for gtsrb2TT100K scenario, use main_train_test.py
...
def train(e):
  ...
  for i, (input, target, template) in enumerate(trainloader):
    ...
    loss4 = (mu-mu_temp).norm(dim=1,p=2).mean()#((mu-mu_temp).pow(2) + 1e-8).sum().sqrt()* 1.0 (mu-mu_temp).norm(dim=1,p=2).sum()#
